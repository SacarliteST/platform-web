...
| MOD-002a | IdentityService: реестр клиентов | P0 | Backlog | MOD-002 | Спека — [`2026-09-04-identity-token-exchange.md`](./backend-requirements/2026-09-04-identity-token-exchange.md). `Clients` (`client_id`, `client_secret_hash`, `allowed_audiences`); одна запись — `education-core` с `allowed_audiences=["sql-module-api"]` |
| MOD-002b | IdentityService: эндпоинт Token Exchange | P0 | Backlog | MOD-002a | Та же спека. `POST /api/v1/auth/token/exchange` (Basic client_id:secret, `subjectToken`+`audience` → новый JWT с целевой `aud`, короче TTL, без refresh) |
