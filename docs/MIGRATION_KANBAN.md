...
| MOD-010 | platform-web: привязка модуля к практике (преподаватель) | P0 | Backlog | MOD-006, MOD-006b | `GET /practicals/{id}` — текущая привязка (`kind`, `moduleBinding`); `GET /practical-modules/{id}/tasks` — каталог; `PUT /practicals/{id}/module {practicalModuleId, externalTaskRef, triesCount, timeLimitMinutes?}` → 204/403/404/**409** (`409` — по практике уже есть работа студентов; UI предлагает привязку только для пустой практики) |
