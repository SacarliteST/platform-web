...
> **Статус 2026-09-07: закрыто.** Соседи внедрили штатный `Platform`-профиль
> (`SeedSmokeData` + `SmokeDataSeeder` + согласованный ключ/URL), прогон 3
> прошёл 18/18 на чистых контейнерах без правок. Документ оставлен как
> история требований. Единственный побочный баг платформы, найденный на
> прогоне 3 — `SMK-12` (потеря события победной попытки, `PracticeEventHandler`),
> исправлен коммитом `cdbde9b`.
