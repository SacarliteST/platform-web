...
Что ещё открыто по Education: `MOD-006b` (чтение привязки практики для UI),
`MOD-012a` (read-эндпоинт ленты событий) — оба в бэклоге, для экранов `MOD-010/012`.
...
| MOD-006b | Education: чтение привязки практики для UI | P1 | Backlog | MOD-006 | `GET /api/v1/practicals/{id}` (detail) либо расширить ответ списка практик: `kind`, `timeLimitMinutes`, `{practicalModuleId, externalTaskRef}`. Нужно для `MOD-010` (показать, что практика уже внешняя и к чему привязана) |
| MOD-012a | Education: read-эндпоинт ленты событий | P1 | Backlog | MOD-008 | `GET /api/v1/practicals/{practicalId}/module-sessions/{sessionId}/events` → `[{kind, occurredAt, payload}]` (студент — свои; преподаватель — в своём курсе). Нужно для `MOD-012` |
| MOD-009 | platform-web: реестр модулей (экран администратора) | P1 | Backlog | MOD-004 | CRUD `PracticalModule` на `/admin/*` — контракты в [`2026-09-05-sql-module-integration.md`](./backend-requirements/2026-09-05-sql-module-integration.md) §C5; форма БЕЗ поля serviceKey |
| MOD-010 | platform-web: привязка модуля к практике (преподаватель) | P0 | Backlog | MOD-006, MOD-006b | `GET /practical-modules/{id}/tasks` (каталог) + `PUT /practicals/{id}/module {practicalModuleId, externalTaskRef, triesCount, timeLimitMinutes?}` (204/403/404/422); текущую привязку читать через `MOD-006b` |
| MOD-011 | platform-web: запуск внешней практики + гейт попыток + возврат (студент) | P0 | Backlog | MOD-007 | `taskId` — из `GET /practicals/{id}/tasks[0].id`. Гейт по `GET .../module-sessions/current?taskId=` (`session|null, attemptsCount, triesCount, timeLimitMinutes, bestGrade`). «Начать»/«Продолжить» → `POST .../module-sessions {taskId}` → `200 {launchUrl, resumed}` / `409 TriesExhausted` / `502 ModuleUnavailable`; `window.location = launchUrl`. «Прервать» → `POST .../{sid}/abandon` (204/409). Возврат `?session=` → `GET .../module-sessions/{sid}`: `COMPLETED` → оценка + протокол; `ACTIVE` → «обрабатывается», поллинг ~2с (запасной путь); `EXPIRED` → текст по `endReason` |
| MOD-012 | platform-web: просмотр протокола модульной сессии (события) | P1 | Backlog | MOD-008, MOD-012a | лента `PracticalTaskEvent` (`kind` + `payload`) на странице практики (студент + преподаватель), постфактум, по попыткам |
