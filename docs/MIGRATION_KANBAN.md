...
**🔴 Приоритетная очередь — разблокирует backend-команду SqlModule (запрос 2026-09-05).**
Backend SqlModule заблокирован до готовности этих задач; они идут вперёд экранов
`platform-web` (MOD-009/010/012). Строгий порядок критического пути:
**MOD-002c → MOD-006 → MOD-007 → MOD-008**; **MOD-005** — параллельно (готова).
Контракты `push`/`/complete`/Kafka зафиксированы в
[`2026-09-04-practical-modules.md`](./backend-requirements/2026-09-04-practical-modules.md)
§«Подтверждённые контрактные ответы». После этого разблокируются `SQLI-005/006/012–017`
на стороне модуля и фронтовый `/launch` (MOD-013).
...
| MOD-002c | IdentityService: `session_id` в module-токене | P0 | Ready | MOD-002b | 🔴 **Блокер backend-команды SqlModule.** `POST /api/v1/auth/token/exchange` принимает `sessionId` (передаёт Education) и кладёт в выпускаемый `module_access_token`: claim `session_id=<sessionId>`, `sub=<userId студента>`, `aud=sql-module-api`, роль `Student` (как в `subjectToken`). TTL согласован с временем жизни практической сессии — если `expiresAt` сессии раньше стандартного TTL, обрезать по нему; продление — через новый обмен на «Продолжить». Токен без `sessionId` остаётся валидным (standalone). Спека — [`2026-09-04-identity-token-exchange.md`](./backend-requirements/2026-09-04-identity-token-exchange.md) §4 |
...
| MOD-005 | Education: проксирование каталога заданий модуля | P0 | Ready | MOD-004, MOD-013a | 🔴 **Блокер backend-команды SqlModule.** Спека E2. `GET /api/v1/practical-modules/{id}/tasks` (`TeacherOnly`) — сервер-сервер вызов ручки модуля с `X-Service-Key` (секрет в `PracticalModules:<slug>:ServiceKey`, не в `configuration`), таймаут 2–3 с, `502` при недоступности, браузер к API модуля не ходит |
| MOD-006 | Education: `Practical.kind=external`, привязка модуля к практике (1:1) + `triesCount` + `timeLimitMinutes` | P0 | Backlog | MOD-005 | 🔴 **Блокер backend-команды SqlModule.** Спека E3. `PUT /api/v1/practicals/{id}/module { practicalModuleId, externalTaskRef, triesCount, timeLimitMinutes }` (`timeLimitMinutes` — целое или `null`=без лимита), единственный внешний `PracticalTask` |
| MOD-007 | Education: жизненный цикл сессии, гейт попыток, пуш в модуль, Token Exchange, приём оценки | P0 | Backlog | MOD-002c, MOD-006 | 🔴 **Блокер backend-команды SqlModule.** Спека E4–E7, E9, E11 + §«Подтверждённые контрактные ответы». `PracticalModuleSession` (`tryNumber`, `ACTIVE/COMPLETED/EXPIRED`, `end_reason`, `session_key`, `return_url`, `expires_at`). `POST /module-sessions`: продолжение живой `ACTIVE`-сессии (не `409`), гейт `attemptsCount>=triesCount`; **пуш сессии модулю** `POST {module}/module-integration/sessions` (`X-Service-Key`, `{sessionId, sessionKey, userId, taskRef, returnUrl, expiresAt}`) — **до** Token Exchange и сборки `launchUrl`; Token Exchange с `sessionId`; `launchUrl` = `?session=` + `#access_token=` от зарегистрированного origin. `GET .../current` — гейт UI. `POST /module-sessions/{id}/complete` (`X-Service-Key`+`sessionKey`, идемпотентно: повтор → `200`; сессия закрыта → `409` окончательно). `POST .../abandon`. Ленивое истечение по `expires_at` + потолок 24 ч |
| MOD-008 | Education: Kafka-consumer (только события «цифрового следа») | P0 | Backlog | MOD-003, MOD-007 | 🔴 **Блокер backend-команды SqlModule.** Спека E8. Консьюмер `scoodle.practice.events`: key = `sessionId`, сверка `sessionKey` (в обычные логи не писать), `INSERT PracticalTaskEvent (id=eventId, kind, payload) ON CONFLICT (id) DO NOTHING`, события после terminal — отбрасываются. Никаких терминальных переходов (оценка приходит по HTTP в MOD-007). Poison-message: пропустить после 5 неудачных обработок (лог `eventId`+`sessionId`, без `sessionKey`), DLQ-топика в MVP нет. Привести `Education.Contracts.Kafka` к формату `eventId/kind/payload`, удалить `PracticeCompletionMessage` и топик `completion` |
