...
**⚠️ Нужен ещё один ре-экспорт OpenAPI + `npm run api:generate`.** После него убрать
два временных костыля в `platform-web`:
1. `TeacherExternalPractical` — `(response.status as number) === 409` на
   `PUT /practicals/{id}/module` (409-гард добавлен в Education коммитом `784a9e7`
   уже после экспорта `359c517`).
2. `features/module-practice/api/moduleSessionsList.ts` — ручной `useModuleSessionsList`
   заменить на сгенерированный `useListPracticalModuleSessions`
   (`GET /practicals/{id}/module-sessions`, Education коммит `3d7b688`).
...
| MOD-012 | platform-web: просмотр протокола модульной сессии (события) | P1 | Done | MOD-008, MOD-012a, MOD-012b | ✅ Коммит `aa918bb`. `SessionEventsFeed` — общий таймлайн (`kind` + время + `payload` в `Spoiler`), 404 скрыт. Студент: под гейтом (последняя сессия) и под статусом возврата (завершённая/прерванная). Преподаватель: `TeacherSessionProtocols` — таблица попыток (студент/№/статус/оценка/начата) → выбор строки открывает ленту. Список попыток — ручной `useModuleSessionsList` (`educationFetch`), заменить на сгенерированный хук после ре-экспорта OpenAPI |
