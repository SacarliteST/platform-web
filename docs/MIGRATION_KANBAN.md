...
| ADM-009 | Live smoke admin: создание пользователя, смена роли, блок/разблок, связывание профиля, аудит | P0 | In progress | ADM-001–ADM-007 | Смоук 2026-09-03: пользователи / фильтры / пагинация, создание, смена ролей (204+событие), блок с причиной, разблок, журнал аудита, настройки+health — **через IdentityService: PASS**. `/auth/me` Education → **200** (после фикса JWT-конфига Education, коммит `2f63af0`). `AdminProfilesPage` → на бэкенде вскрылись 2 бага Education (не фронт): (1) `Identity:Authority`/`Audience`/`RoleClaimType` рассинхрон → 401/403; (2) `GetProfilesAsync` — непереводимый LINQ → 500. Оба исправлены в `2f63af0`; нужен **rebuild + restart Education**, затем повтор экрана профилей |
...
- **Phase 3 — контур администратора готов (2026-09-03).** `ADM-001…007` → Done:
  ...
  `typecheck` + `build` зелёные (JS ~660 kB — route-splitting в `TD-002`).
  `ADM-009` (live smoke) → Ready. `ADM-011` (назначение студентов) → **Blocked**: у админа
  нет эндпоинта для списка курсов/практик — нужно решение владельца. `ADM-008` (справка) — P3.
  Дальше — Phase 4 (контур преподавателя, `TEA-001…017`).
