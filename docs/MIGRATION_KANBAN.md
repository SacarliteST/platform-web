...
| MOD-002 | Утвердить контракт Education (спека backend-requirements) | P0 | Done | MOD-001 | [`2026-09-04-practical-modules.md`](./backend-requirements/2026-09-04-practical-modules.md) — модель данных, эндпоинты E1–E9 (реестр/каталог/привязка/сессия/attach/Kafka-consumer/best-of-N/TTL), приёмка |
...
| MOD-013 | sql-module-web: маршрут `/launch` + `TokenProvider: handoff` | P0 | Backlog | MOD-007 | Спека: [`2026-09-05-sql-module-integration.md`](./backend-requirements/2026-09-05-sql-module-integration.md) §F1-F3. Читает `session/task/return_url` из query, `access_token` из фрагмента (→ `sessionStorage` через новый `handoff-token-provider`, чистит URL через `history.replaceState`); attach-релей к SqlModule (MOD-014); по завершении → `window.location = return_url` |
| MOD-013a | SqlModule: ручка каталога заданий (`/api/v1/module-integration/tasks-catalog`) | P0 | Backlog | — | Спека: [`2026-09-05-sql-module-integration.md`](./backend-requirements/2026-09-05-sql-module-integration.md) §S1. Список опубликованных заданий (`ref/name/description`) для проксирования ядром (MOD-005); аутентификация — service-key (решение зафиксировано в спеке) |
| MOD-014 | SqlModule: attach-релей + Kafka-producer (события попыток + завершение с `grade`) | P0 | Backlog | MOD-003, MOD-007 | Спека: [`2026-09-05-sql-module-integration.md`](./backend-requirements/2026-09-05-sql-module-integration.md) §S2-S3. Новый `POST .../sessions/{id}/attach` вызывает Education сервер-сервер, хранит `moduleToken` локально; продюсер в `scoodle.practice.*` (формат 1:1 с `Education.Contracts.Kafka`); критерий completion — открытый вопрос в спеке, рекомендация: первая верная попытка |
| MOD-014a | SqlModule: собственная `Audience` (Token Exchange) | P0 | Backlog | MOD-002b, MOD-013 | Спека: [`2026-09-05-sql-module-integration.md`](./backend-requirements/2026-09-05-sql-module-integration.md) §S4. `Authority`=IdentityService, `Audience="sql-module-api"` вместо общего `scoodle-api`; ломает текущий standalone-логин — решение по нему зафиксировать перед реализацией |
