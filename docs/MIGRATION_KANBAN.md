...
| ADM-009 | Live smoke admin: создание пользователя, смена роли, блок/разблок, связывание профиля, аудит | P0 | In progress | ADM-001–ADM-007 | Смоук 2026-09-03: список / фильтры / пагинация, создание пользователя, смена ролей (204 + событие), блок с причиной, разблок, журнал аудита, настройки с живым health-check — **всё через IdentityService: PASS**. `AdminProfilesPage` (Education `admin/profiles`) → **401**: у `Education` dev-конфига был неверный `Identity:Authority`/`Audience` → правка `appsettings.Development.json` (Authority `http://localhost:5101`, Audience `scoodle-api`, `RequireHttpsMetadata=false`), нужен рестарт Education и повтор |
