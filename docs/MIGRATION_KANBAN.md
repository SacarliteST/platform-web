...
| MOD-008 | Education: Kafka-consumer (только события «цифрового следа») | P0 | In progress | MOD-003, MOD-007 | 🔴 **Блокер backend-команды SqlModule.** Спека E8. **Контракт готов** (коммит `3522126`): `Education.Contracts.Kafka` → `PracticeEventMessage { SessionId, SessionKey, EventId, Kind, OccurredAt, Payload }`, `PracticeCompletionMessage`/топик `completion` удалены. **Осталось:** консьюмер в `Education.Application` — key = `sessionId`, сверка `sessionKey` (в обычные логи не писать), `INSERT PracticalTaskEvent (id=eventId, kind, payload) ON CONFLICT (id) DO NOTHING`, события после terminal — отбрасываются, без терминальных переходов. Poison-message: пропустить после 5 неудачных обработок (лог `eventId`+`sessionId`, без `sessionKey`), DLQ-топика в MVP нет |
